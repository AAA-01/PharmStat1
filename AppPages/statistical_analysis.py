...
def show(language=None):
    ...
    uploaded_file = st.file_uploader(
        "Загрузите Excel-файл", type=["xlsx", "xls"])
    ...
    if uploaded_file:
        ...
        try:
            ...
            st.info(f"""
            **Нормальные распределения групп** (p > 0.05): {', '.join(normal_groups)}

            **Ненормальные распределения групп** (p < 0.05): {', '.join(non_normal_groups)}
            ...
            if result['levene_p'] is not None:
                ...
                if result['levene_p'] > result['alpha']:
                    st.success(
                        f"Дисперсии однородны (тест Левена, p = {result['levene_p']:.4f})")
                else:
                    st.warning(
                        f"Дисперсии неоднородны (тест Левена, p = {result['levene_p']:.4f})")

            # Итоговый вывод
            if result['p_value'] < alpha:
                st.success(
                    "Обнаружены статистически значимые различия (p < alpha).")
            else:
                st.info("Статистически значимых различий не выявлено (p ≥ alpha).")

            # Дополнительная статистика по группам
            # Инициализация переменной заранее
            all_data = []

                # Заполняем список только при показе
            ...
            with st.subheader("Полная таблица статистических показателей"):
                        st.dataframe(full_df, use_container_width=True, height=None)
